    ...
    <header className="sticky top-0 z-50 border-b border-warm-200/60 bg-cream/80 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
        <Link href={`/${locale}`} className="flex items-center gap-2">
          ...
          <span className="font-display text-xl font-light italic text-rose lg:text-2xl">
            &amp; More
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {navItems.map((item) => (
            <Link
              ...
              className="rounded-lg px-3.5 py-2 text-sm font-medium text-ink/70 transition-colors hover:bg-warm-100 hover:text-ink"
            >
              {dict.nav[item.key]}
            </Link>
          ))}
